...
import data_loader
import hundredtwenty.net as net
import torch
...
def train(model, optimizer, loss_fn, train_data, trsize, batch_size): #lets see what else
    ...
    labels=torch.reshape(train_data,(trsize,1728000)) #120*120*120=1728000

    with tqdm(total=trsize) as pbar:
        for t in range(0, trsize, batch_size): #percorrer o dataset

            inputs=torch.Tensor(batch_size,1,120,120,120).cuda()
            targets=torch.Tensor(batch_size,120*120*120).cuda()
